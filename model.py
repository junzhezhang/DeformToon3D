...
class Decoder(nn.Module):
    def __init__(self, model_opt, blur_kernel=[1, 3, 3, 1], style_opt=None):
        super().__init__()
        ...
        self.adain_layer_idx = []

        if self.style_opt.adaptive_style_mixing:
            # implement res and t_c
            width = 512
            t_c_layers = [EqualLinear(256, width, lr_mul=0.01, activation="fused_lrelu")]
            for i in range(self.style_opt.adaptive_style_mixing_block_depth-1):
                t_c_layers.append(EqualLinear(width, width, lr_mul=0.01, activation="fused_lrelu"))
            self.t_c = nn.Sequential(*t_c_layers)
            self.n_res = 0
            self.res_flag = [0] * 10  # 1, 4x2, 1, conv0, conv1-4, last rgb; rgb share the next conv1's res
            if len(self.style_opt.adaptive_style_mixing_blocks) > 0:
                self.res = nn.ModuleList()
                self.res_blocks = self.style_opt.adaptive_style_mixing_blocks.split('+')
                if 'conv0' in self.res_blocks:
                    self.res_flag[0] = 1
                    self.n_res += 1
                if 'conv1' in self.res_blocks:
                    self.res_flag[1] = 1
                    self.res_flag[2] = 1
                    self.n_res += 2
                if 'conv2' in self.res_blocks:
                    self.res_flag[3] = 1
                    self.res_flag[4] = 1
                    self.n_res += 2
                if 'conv3' in self.res_blocks:
                    self.res_flag[5] = 1
                    self.res_flag[6] = 1
                    self.n_res += 2
                if 'conv4' in self.res_blocks:
                    self.res_flag[7] = 1
                    self.res_flag[8] = 1
                    self.res_flag[9] = 1
                    self.n_res += 3
                self.res_index = [sum(self.res_flag[:i+1])-1 if self.res_flag[i] == 1 else -1 for i in range(len(self.res_flag)) ]
    ...
    def forward(self, features, styles, rgbd_in=None, transform=None,
                ...
                randomize_noise=True, mesh_path=None, exstyle=None,
                # interp_weights=[1]*18, # weight vector for style combination of two paths
                ):
        """
        if input_is_latent:
            styles: W of list of len=1, [1, 10, 512]
            latent: W [1, 10, 512]
        else:
        ...
        if self.style_opt.adaptive_style_mixing:
            ...
            exstyle = self.t_c(exstyle)
            if exstyle.ndim < 3: 
                # resstyles = self.t_c(exstyle).unsqueeze(1).repeat(1, self.generator.n_latent, 1)
                adastyles = exstyle.unsqueeze(1).repeat(1, latent.shape[1], 1)
            else:
                # nB, nL, nD = exstyle.shape
                # resstyles = self.t_c(exstyle.reshape(nB*nL, nD)).reshape(nB, nL, nD)
                adastyles = exstyle
        
        if self.style_opt.adaptive_style_mixing and self.res_flag[0] == 1:
            k = self.res_index[0]
            out = self.conv1(features, interp_weights[0] * self.res[k](adastyles[:, 0]) +
                              (1-interp_weights[0]) * latent[:, 0], noise=noise[0],
                            transform=transform, mesh_path=mesh_path)
        else:
            out = self.conv1(features, latent[:, 0], noise=noise[0],
                        transform=transform, mesh_path=mesh_path)
        
        if self.style_opt.adaptive_style_mixing and self.res_flag[1] == 1:
            k = self.res_index[1]
            skip = self.to_rgb1(out, interp_weights[1] * self.res[k](adastyles[:, 0]) +
                              (1-interp_weights[0]) * latent[:, 1], skip=rgbd_in)
        else:
            skip = self.to_rgb1(out, latent[:, 1], skip=rgbd_in)
        
        # NOTE that to_rgb and subsequent conv1 share the same noise
        i = 1
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            self.convs[::2], self.convs[1::2], noise[1::2], noise[2::2], self.to_rgbs
        ):
            if self.style_opt.adaptive_style_mixing and self.res_flag[i] == 1:
                k = self.res_index[i]
                out = conv1(out, interp_weights[i] * self.res[k](adastyles[:, i]) +
            ...
            if self.style_opt.adaptive_style_mixing and self.res_flag[i+1] == 1:
                k = self.res_index[i+1]
                out = conv2(out, interp_weights[i+1] * self.res[k](adastyles[:, i+1]) + 
                ...
                out = conv2(out, latent[:, i + 1], noise=noise2,
                                    transform=transform, mesh_path=mesh_path)
            if self.style_opt.adaptive_style_mixing and self.res_flag[i+2] == 1:
                k = self.res_index[i+2]
                skip = to_rgb(out, interp_weights[i+2] * self.res[k](adastyles[:, i+2]) +
                              (1-interp_weights[i+2]) * latent[:, i + 2], skip=skip)
            else:
...
class Generator(nn.Module):
    def __init__(self, model_opt, renderer_opt, style_opt=None, blur_kernel=[1, 3, 3, 1], ema=False, full_pipeline=True):
        super().__init__()
        self.size = model_opt.size
        ...
        # volume renderer
        thumb_im_size = model_opt.renderer_spatial_output_dim
        self.renderer = VolumeFeatureRenderer(renderer_opt, style_opt=style_opt, style_dim=self.style_dim,
                                              out_im_res=thumb_im_size)

        if self.full_pipeline:
            self.decoder = Decoder(model_opt, style_opt=style_opt)
    ...
    def forward(self, styles, cam_poses, focals, near=0.88, far=1.12, return_latents=False,
                ...
                project_noise=False, mesh_path=None, inversion_latent_type='z', 
                exstyle=None, exstyle_g0=None, exstyle_g1=None):
        """
        if inversion_latent_type == 'z':
            styles: raw z, list of len=1,  with shape [batch_size, style_dim(256)]
            latent: W, list of len=1, with shape [batch_size, (256)]
        if inversion_latent_type == 'w' or inversion_latent_type == 'w+':
        exstyle_g0 and exstyle_g1 are used for interpolation
        """
        # if exstyle is not None:
        #     ex = exstyle[0,:1].detach().cpu().numpy().tolist()
        # else:
        #     ex = 0
        # if exstyle_g0 is not None:
        #     ex_g0 = exstyle_g0[0,:1].detach().cpu().numpy().tolist()
        #     ex_g1 = exstyle_g1[0,:1].detach().cpu().numpy().tolist()
        # else:
        #     ex_g0 = 0
        #     ex_g1 = 0
        # swap geo and encoder
            ...
            if inversion_latent_type == 'z':
                with torch.set_grad_enabled(self.is_train and self.train_renderer):
                    ...
                    thumb_rgb, features, sdf, mask, xyz, eikonal_term, others = self.renderer(cam_poses, 
                        focals, near, far, styles=latent[0], return_eikonal=return_eikonal, raw_latent_z=styles, exstyle=exstyle_g0)
                
                if self.full_pipeline:
                    c_cat = torch.cat([latent[0], exstyle_g1], dim=-1)
                    exstyle_target_mapped =  self.renderer.exstyle_mapping_net(c_cat)
                    rgb, decoder_latent = self.decoder(features, latent,
                                                    ...
                                                    mesh_path=mesh_path, exstyle=exstyle_target_mapped)

                else:
                    rgb = None
                out = (rgb, thumb_rgb, others)
                return out
            elif inversion_latent_type == 'w' or inversion_latent_type == 'w+':
                input_is_latent = True
                with torch.set_grad_enabled(self.is_train and self.train_renderer):
                    thumb_rgb, features, sdf, mask, xyz, eikonal_term, others = self.renderer(cam_poses, 
                        focals, near, far, styles=styles[0], return_eikonal=return_eikonal, raw_latent_z=styles, exstyle=exstyle_g0)
                
                if self.full_pipeline:
                    c_cat = torch.cat([styles[0][:,0,:], exstyle_g1], dim=-1)
                    exstyle_target_mapped =  self.renderer.exstyle_mapping_net(c_cat)
                    rgb, decoder_latent = self.decoder(features, [styles[1]],
                                                    transform=cam_poses if project_noise else None,
                                                    return_latents=return_latents,
                                                    inject_index=inject_index, truncation=truncation,
                                                    truncation_latent=truncation_latent, noise=noise,
        ...
        if inversion_latent_type == 'z':
            ...
            # inference with w or w+ latent
            # styles with len=2  torch.Size([1, 9, 256]) [1, 10, 512]
            input_is_latent = True
            with torch.set_grad_enabled(self.is_train and self.train_renderer):
                thumb_rgb, features, sdf, mask, xyz, eikonal_term, others = self.renderer(cam_poses, 
                    focals, near, far, styles=styles[0], return_eikonal=return_eikonal, raw_latent_z=styles, exstyle=exstyle)
            if self.full_pipeline:
                exstyle = others['mapped_exstyle']
                rgb, decoder_latent = self.decoder(features, [styles[1]],
                                                transform=cam_poses if project_noise else None,
                ...
                rgb = None
        elif 'w+half' in inversion_latent_type:
            which_latent_channel = int(inversion_latent_type.split('_')[-1])
            with torch.set_grad_enabled(self.is_train and self.train_renderer):
                thumb_rgb, features, sdf, mask, xyz, eikonal_term, others = self.renderer(cam_poses, 
                    focals, near, far, styles=styles[0], return_eikonal=return_eikonal, raw_latent_z=styles, exstyle=exstyle)
            if self.full_pipeline:
                exstyle = others['mapped_exstyle']
                rgb, decoder_latent = self.decoder(features, [styles[0][:,which_latent_channel]],
                                                transform=cam_poses if project_noise else None,
                                                return_latents=return_latents,
                                                inject_index=inject_index, truncation=truncation,
                                                truncation_latent=truncation_latent, noise=noise,
