...
class TestOptions():
    def __init__(self):
        ...
        self.parser.add_argument("--save_overview", action="store_true", help="transfer the color of the style")
...
if __name__ == "__main__":
    ...
    pathnames = sorted([p for p in pathnames if 'vtoonify' not in p]) # exclude toonified images in case process multiple rounds
            ...
            if args.video:
                ...
                for i in tqdm(range(num)):
                    ...
                    if first_valid_frame:
                        ...
                        with torch.no_grad():
                            I = align_face(frame, landmarkpredictor) # (248, 248, 3) -->
                ...
                # savename = os.path.join(args.content, basename + '_vtoonify_' +  args.backbone[0] + '.png')
                ...
                # import pdb; pdb.set_trace()
                if frame.shape[0] == 512: # (512, 512, 3)
                ...
                with torch.no_grad():
                    # import pdb; pdb.set_trace()
                    I = align_face(frame, landmarkpredictor) # frame
                    I = transform(I).unsqueeze(dim=0).to(device)
                    s_w = pspencoder(I)
                    s_w = vtoonify.zplus2wplus(s_w)
                    if vtoonify.backbone == 'dualstylegan':
