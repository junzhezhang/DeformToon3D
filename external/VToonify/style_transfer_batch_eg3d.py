...
class TestOptions():
    def __init__(self):
        ...
        self.parser.add_argument("--include_input", action="store_true", help="transfer the color of the style")
    ...
    for i, filename in enumerate(pathnames):
        ...
        try:
            if args.video:
                ...
                y_tilde = F.interpolate(y_tilde, size=512, mode='bilinear', align_corners=False)
                save_image(y_tilde[0].cpu(), savename)
                print(f'Style Transfer {basename} successfully!')
        except:
